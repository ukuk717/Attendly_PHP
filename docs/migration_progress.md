...
- MAIL_TRANSPORT を追加し、ロリポ環境で mail() を使う/使わないを .env で切替可能にした（デフォルト log）。mail() を呼ばない場合もログにはハッシュ化した宛先/件名を残す。
- メールログのローテーションしきい値（MAIL_LOG_MAX_KB, 下限64KB）を追加し、容量超過時は .bak へ退避してログ肥大/PII残留を抑制。
