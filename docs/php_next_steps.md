...
- `styles.css` を Slim ルートで配信（/styles.css）し 404 を解消。ヘッダのロゴリンクをログイン状態に応じて `/dashboard` または `/login` へ遷移するよう変更。
...
- RateLimiter を APCu 対応（CLI 含む）にし、APCuなしは単一プロセス用にフォールバック。APCu ではウィンドウごとのカウンタでレースを抑制。
