<div class="auth-container">
  <section class="card auth-card">
    <h2>メールアドレスの確認</h2>
    <p class="form-note">
      <code class="code-large"><?= $e($email ?? '') ?></code> に 6 桁の確認コードを送信しました。10 分以内に入力して登録を完了してください。
    </p>
    <form method="post" action="/register/verify" class="form">
      ...
      <label class="form-field" for="token">
        <span>確認コード</span>
        <input type="text" id="token" name="token" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" required autocomplete="one-time-code">
      </label>
